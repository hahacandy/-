...
createCover(){
	...
	Loop, 3{
		ImageSearch, vx, vy, nx, %searchY%, nx+clientSizeW, ny+clientSizeH, *90 %searchImageName%.png
		if(ErrorLevel == 0){
			searchY := vy+100
			vvx := vx - corverX
			vvy := vy - corverY
			...
			ImageSearch, vx, vy, nx, %searchY%, nx+clientSizeW, ny+clientSizeH, *90 %searchImageName2%.png
			if(ErrorLevel == 0){
				searchY := vy+100
				vvx := vx - corverX
				vvy := vy - corverY
...
isChange(){
	...
	while(true)
	{
		...
		tooltip, %cnt%
		WinGetPos, nx, ny,,, %clientName%
		ImageSearch, vx, vy, nx, ny, nx+180, ny+80, *80 current.png
		if(ErrorLevel == 0){
		}else{
			tooltip
			break
